...
# Plot 1: survElec
x, y1, y2    = data[0][0][:, 0], data[0][0][:, 1], data[0][0][:, 2]
y1LEC, y2LEC = data[0][1][:, 1], data[0][1][:, 2]
...
# Plot 2: survPix
x_pix, y1_pix = data[1][0][:, 0], data[1][0][:, 1]
y1LEC_pix = data[1][1][:, 1]
ax1.plot(x_pix, y1_pix, "-o", label="Binomial mask", color="cyan")
ax1.plot(x_pix, y1LEC_pix, "-", label="LEC", color="violet")
...
rates2 = np.divide(y2, y1_pix, out=np.zeros_like(y2), where=y1_pix != 0)
rates1LEC = np.divide(y1LEC, y1LEC_pix, out=np.zeros_like(y1LEC), where=y1LEC_pix != 0)
rates2LEC = np.divide(y2LEC, y1LEC_pix, out=np.zeros_like(y2LEC), where=y1LEC_pix != 0)
